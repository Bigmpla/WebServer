...
void BlockQueue<T>::push_back(const T&item){
    ...
    while(deq_.size() >= capacity_){
        condProducer_.wait(locker);//释放锁，进入等待，被唤醒后重新获取锁
    }
    deq_.push_back(item);
    condConsumer_.notify_one();
}
...
void BlockQueue<T>::push_front(const T&item){
    unique_lock<mutex>locker(mtx_);
    while(deq_.size() >= capacity_){
        condProducer_.wait(locker);//日志队列为满，生产者阻塞，等待空位
    }
    deq_.push_front(item);
    condConsumer_.notify_one();//有新的日志了，唤醒一个消费者
}
...
bool BlockQueue<T>::pop(T&item){
    unique_lock<mutex>locker(mtx_);
    while(deq_.empty()){
        condConsumer_.wait(locker);//日志队列为空，消费者阻塞，等待被notify唤醒
    }
    item = deq_.front();
    deq_.pop_front();
